...
const plantilles = [
  { nom: 'Plantilla Document (PDF)', desc: 'Estructura completa del document de projecte', icon: BookOpen, url: import.meta.env.BASE_URL + 'plantilles/plantilla_document.pdf' },
  { nom: 'Guia del Prototip', desc: 'Passos per desenvolupar el prototip', icon: Wrench, url: import.meta.env.BASE_URL + 'plantilles/guia_prototip.pdf' },
  { nom: 'Guió de la Presentació', desc: 'Estructura i consells per a l\'exposició oral', icon: Video, url: import.meta.env.BASE_URL + 'plantilles/guio_presentacio.md' },
  { nom: 'Rúbrica Completa (PDF)', desc: 'Criteris detallats d\'avaluació', icon: BookOpen, url: import.meta.env.BASE_URL + 'plantilles/rubrica_evaluacio.md' },
]
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {plantilles.map((pl, i) =>
            pl.url ? (
              ...
              <button
                key={i}
                className="flex items-start space-x-3 bg-white/10 hover:bg-white/20 p-4 rounded-lg transition-colors text-left opacity-60 cursor-not-allowed"
                disabled
              >
                <pl.icon className="w-6 h-6 flex-shrink-0 mt-1" />
                <div>
                  <p className="font-medium">{pl.nom}</p>
                  <p className="text-sm text-white/80">{pl.desc} (pròximament)</p>
                </div>
              </button>
            )
          )}
